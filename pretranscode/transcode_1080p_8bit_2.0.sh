...
find "$MoviesDir" -type f \( -iname "*.mkv" -o -iname "*.mp4" \) | while read -r file; do
    ...
    if [[ "$base" =~ \[1080p\ 8bit\ 2\.0\]$ ]]; then
        continue
    fi

    if [[ "$base" =~ \[[0-9]{3,4}p.*\]$ ]]; then
        outName="${base/\[[0-9]\{3,4\}p.*\]/[1080p 8bit 2.0]}"
    else
        outName="$base - [1080p 8bit 2.0]"
    fi
    ...
    ffmpeg -hide_banner -stats -n -i "$file" -map 0 \
        -c:v libx264 -preset slow -crf 20 -pix_fmt yuv420p -vf "scale=-2:1080" \
