...
Get-ChildItem -Path $MoviesDir -Recurse -Include *.mkv, *.mp4 | ForEach-Object {
    ...
    if ($name -match "\[1440p 8bit 2\.0\]$") { return }

    if ($name -match "\[\d{3,4}p.*?\]$") {
        $outName = $name -replace "\[\d{3,4}p.*?\]$", "[1440p 8bit 2.0]"
    } else {
        $outName = "$name - [1440p 8bit 2.0]"
    }
    ...
    if ($height -le 1440) {
        Write-Host "Skipping (<=1440p): $file"
        return
    }
    ...
    & ffmpeg -hide_banner -stats -n -i "$file" -map 0 `
        -c:v h264_nvenc -preset p7 -pix_fmt yuv420p -vf "scale=-2:1440" `
